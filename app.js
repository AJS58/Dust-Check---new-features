...
if($('refineFromManualBtn')) $('refineFromManualBtn').addEventListener('click', refineDetectionFromManualMarks);
...
$('resetBtn').addEventListener('click', ()=>{ if(originalData){spots=[];oilSpots=[];gritSpots=[];manualSpots=[];cleanData=null;cleanMode=false;overlay=true;ctx.putImageData(originalData,0,0);render();updateResults(null);try{updateComparePanel();}catch(e){}} });
$('clearManual').addEventListener('click', ()=>{manualSpots=[]; cleanData=null; render(); updateResults(summary());
  try{ updateComparePanel(); }catch(e){}
  try{ updateComparePanel(); }catch(e){ console.warn(e); } try{updateComparePanel();}catch(e){} });
...
canvas.addEventListener('click', e=>{
  ...
  if(activeTool==='mark'){
    manualSpots.push({x,y,r:16,area:800,manual:true});
    cleanData=null;
    cleanMode=false;
    overlay=true;
    render();
...
function detectLargeTranslucentOilPass(gray,w,h,existing,mode,strength){
  const results=[];
  const occupied=(x,y,r)=>existing.some(s=>Math.hypot(s.x-x,s.y-y)<Math.max(r,s.r||8)*0.72);
  const minDim=Math.min(w,h);
  const aggressive=mode.includes('aggressive');
  const step=aggressive ? 6 : 8;
  const minR=Math.max(12,Math.round(minDim/115));
  const maxR=Math.max(minR+22,Math.round(minDim/7.5));
    ...
    for(let yy=-r2; yy<=r2; yy+=skip){
      for(let xx=-r2; xx<=r2; xx+=skip){
        ...
        if(d>=r1 && d<=r2){
          const x=Math.round(cx+xx), y=Math.round(cy+yy);
          if(x>=0&&x<w&&y>=0&&y<h){
            const v=gray[y*w+x]; sum+=v; sum2+=v*v; n++; if(v<min)min=v; if(v>max)max=v;
          }
        }
      }
    }
    if(!n) return {mean:0,sd:999,range:999,n:0};
    const mean=sum/n;
    return {mean,sd:Math.sqrt(Math.max(0,sum2/n-mean*mean)),range:max-min,n};
  }

  for(let y=maxR+4; y<h-maxR-4; y+=step){
    for(let x=maxR+4; x<w-maxR-4; x+=step){
      if(occupied(x,y,maxR*0.55)) continue;
      let best=null;
      for(let r=minR; r<=maxR; r+=Math.max(4,Math.round(r/6))){
        const centre=annulus(x,y,0,Math.max(3,Math.round(r*.25)),2);
        const inner=annulus(x,y,Math.round(r*.32),Math.round(r*.58),2);
        const ring=annulus(x,y,Math.round(r*.68),Math.round(r*1.05),2);
        const outer=annulus(x,y,Math.round(r*1.25),Math.round(r*1.72),3);
        if(!centre.n || !ring.n || !outer.n) continue;

        const cR=Math.abs(centre.mean-ring.mean);
        const iR=Math.abs(inner.mean-ring.mean);
        const rO=Math.abs(ring.mean-outer.mean);
        const structure=cR+iR+rO;
        const cloudPenalty=Math.max(0, outer.sd-22)*0.65 + Math.max(0, outer.range-70)*0.08;
        const softHaloBonus=(ring.sd<26 && outer.sd<36) ? 8 : 0;
        const edgeBonus=(x<w*.18 || x>w*.82 || y<h*.18 || y>h*.82) ? 3 : 0;
        const score=structure*2.25 + softHaloBonus + edgeBonus - cloudPenalty;
        const threshold=aggressive ? 10.5 : 13.5;

        if(structure>(aggressive?3.8:5.0) && score>threshold){
          const cand={x,y,r:r*1.08,area:Math.PI*r*r,bw:r*2,bh:r*2,oil:true,halo:true,translucent:true,score};
  ...
  const final=[];
  for(const c of results){
    if(!final.some(o=>Math.hypot(o.x-c.x,o.y-c.y)<Math.max(o.r,c.r)*.86)){
      final.push(c);
      if(final.length>70) break;
    }
  }
  return final;
}
...
function imageDataToScaledDataUrl(imageData, maxSide=900){
  if(!imageData) return '';
...
function canvasToScaledDataUrl(sourceCanvas, maxSide=900){
...
function updateCompareSlider(){
  ...
  if(slider&&wrap) wrap.style.width=(Number(slider.value||50))+'%';
}

function setCompareLoading(isLoading){
  const panel=document.querySelector('.comparePanel');
  if(panel) panel.classList.toggle('compareLoading', !!isLoading);
}

function updateComparePanelNow(){
  if(!originalData) return;
  ...
  const oldClean=cleanMode;
  const oldOverlay=overlay;

  cleanMode=false;
  overlay=true;
  render();
  before.src=canvasToScaledDataUrl(canvas,900);

  if(!cleanData) createCleanPreview(false);
  after.src=imageDataToScaledDataUrl(cleanData,900);

  cleanMode=oldClean;
  overlay=oldOverlay;
  render();

  if(!slider.dataset.boundCompare){
    slider.addEventListener('input',updateCompareSlider);
    slider.addEventListener('change',updateCompareSlider);
    slider.dataset.boundCompare='1';
  }
  updateCompareSlider();
  compareDirty=false;
...
function updateComparePanel(){
  compareDirty=true;
  if(compareUpdateTimer) clearTimeout(compareUpdateTimer);
  compareUpdateTimer=setTimeout(()=>{
    try{ updateComparePanelNow(); }catch(e){ console.warn(e); setCompareLoading(false); }
  },180);
}

function createCleanPreview(setView=true){
  if(!originalData) return;
  ...
  cleanData=out; if(setView){ cleanMode=true; render(); }
...
function summary(){
  ...
  const manualAssistedCount=all.filter(s=>s.manualAssisted).length;
  ...
  return {count,small,medium,large:heavy,oilCount,gritCount,faintCount,manualAssistedCount,batchConfidence:lastBatchSummary?lastBatchSummary.fixed.confidence:null,batchMatches:lastBatchSummary?lastBatchSummary.fixed.matched:0,sev,rec,pattern,coverage,physicalCoverage,healthScore,meaningfulDetection};
...
// Final compare launch fix binding
(function(){
  const slider=document.getElementById('compareSlider');
  if(slider && !slider.dataset.boundCompare){
    slider.addEventListener('input', updateCompareSlider);
    slider.addEventListener('change', updateCompareSlider);
    slider.dataset.boundCompare='1';
  }
  setTimeout(()=>{ try{ updateCompareSlider(); }catch(e){} }, 100);
})();

// Final approved visual compare binding
(function(){
  const slider=document.getElementById('compareSlider');
  if(slider && !slider.dataset.boundCompare){
    slider.addEventListener('input',updateCompareSlider);
