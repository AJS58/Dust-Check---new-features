...
function imageDataToDataUrlSafe(imageData){
  if(!imageData) return '';
  const c=document.createElement('canvas');
...
function updateComparePanel(){
  ...
  const prevCleanMode=cleanMode;
  const prevOverlay=overlay;

  cleanMode=false;
  overlay=true;
  render();
  before.src = canvas.toDataURL('image/png');

  if(!cleanData) createCleanPreview(false);
  after.src = imageDataToDataUrlSafe(cleanData);

  cleanMode=prevCleanMode;
  overlay=prevOverlay;
  render();
